...
import me.alanx.ecomer.core.cms.utils.ProductImageCropUtils;
import me.alanx.ecomer.core.cms.utils.ProductImageSizeUtils;
...
/*				//default small
		...
		String slargeImageHeight = env.getProperty(PRODUCT_IMAGE_HEIGHT_SIZE);
		String slargeImageWidth = env.getProperty(PRODUCT_IMAGE_WIDTH_SIZE);
		...
		if(!StringUtils.isBlank(slargeImageHeight) && !StringUtils.isBlank(slargeImageWidth)) { //&& !StringUtils.isBlank(ssmallImageHeight) && !StringUtils.isBlank(ssmallImageWidth)) {
			...
			if(!StringUtils.isBlank(env.getProperty(CROP_UPLOADED_IMAGES)) && env.getProperty(CROP_UPLOADED_IMAGES).equals(Constants.TRUE)) {
					//crop image
