...
import me.alanx.ecomer.core.services.shipping.Packaging;
...
public class DefaultPackagingImpl implements Packaging {
	...
	public List<PackageDetails> getBoxPackagesDetails(
		...
		for(ShippingProduct shippingProduct : products){
			...
			Set<ProductOptionPrice> attrs = shippingProduct.getProduct().getOptions();
			...
			if (attrs != null && attrs.size() > 0) {
				for(ProductOptionPrice attribute : attrs) {
					if(attribute.getWeight()!=null) {
						w = w.add(attribute.getWeight());
					}
				}
			}
			...
			if (qty > 1) {
				...
				for (int i = 1; i <= qty; i++) {
					...
					temp.setOptions(product.getOptions());
		...
		for(ShippingProduct shippingProduct : products) {
			...
			if (attributes != null && attributes.size() > 0) {
				for(ProductOptionPrice attribute : attributes) {
					if(attribute.getAdditionalWeight()!=null) {
						w = w.add(attribute.getWeight());
					}
				}
			}
