package me.alanx.ecomer.web.store.controller.category;
...
import me.alanx.ecomer.web.dto.catalog.ProductList;
...
import me.alanx.ecomer.web.populator.manufacturer.ReadableManufacturerPopulator;
...
public class ShoppingCategoryController {
	...
	private List<ReadableManufacturer> getManufacturers(MerchantStore store, List<Long> ids, Language language) throws Exception {
		List<ReadableManufacturer> manufacturerList = null;
		List<Manufacturer> manufacturers = manufacturerService.listByProductsByCategoriesId(store, ids, language);
	...
	private ProductList getProducts(final int start, final int max, final String store, final String language, final String category, final List<QueryFilter> filters, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			me.alanx.ecomer.core.model.catalog.product.query.ProductList products = productService.listByStore(merchantStore, lang, productCriteria);
			...
			JsonUtils.writeJson(productList, null);
