package me.alanx.ecomer.web.store.controller.product;
...
import me.alanx.ecomer.web.dto.catalog.category.Attribute;
import me.alanx.ecomer.web.dto.catalog.category.AttributeValue;
...
import me.alanx.ecomer.web.dto.shop.Breadcrumb;
import me.alanx.ecomer.web.dto.shop.PageInformation;
import me.alanx.ecomer.web.populator.catalog.ReadableFinalPricePopulator;
...
import me.alanx.ecomer.web.utils.PageBuilderUtils;
...
public class ShopProductController {
	...
	public String display(final String reference, final String friendlyUrl, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
		...
		//JsonUtils.writeJson(product, null);
		...
		Set<ProductOptionPrice> attributes = product.getOptions();
		...
		if(!CollectionUtils.isEmpty(attributes)) {
			for(ProductOptionPrice attribute : attributes) {
				...
				if(attribute.isDisplayOnly()==true) {//read only attribute
				...
				attrValue.setDefaultAttribute(attribute.isDefault());
				...
				if(attribute.getOptionPrice()!=null && attribute.getOptionPrice().doubleValue()>0) {
					String formatedPrice = pricingService.getDisplayAmount(attribute.getOptionPrice(), store);
	...
	ReadableProductPrice calculatePrice(@RequestParam(value="attributeIds[]") Long[] attributeIds, @PathVariable final Long productId, final HttpServletRequest request, final HttpServletResponse response, final Locale locale) throws Exception {
		...
		List<ProductOptionPrice> attributes = productAttributeService.getByAttributeIds(store, product, ids);      
	...
	private Attribute createAttribute(ProductOptionPrice productAttribute, Language language) {
