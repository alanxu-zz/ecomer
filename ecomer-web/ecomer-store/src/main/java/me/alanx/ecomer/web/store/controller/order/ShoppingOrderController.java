...
import me.alanx.ecomer.web.dto.customer.AnonymousCustomer;
import me.alanx.ecomer.web.dto.customer.PersistableCustomer;
import me.alanx.ecomer.web.dto.customer.ReadableDelivery;
import me.alanx.ecomer.web.dto.order.ReadableOrderTotal;
import me.alanx.ecomer.web.dto.order.ReadableShippingSummary;
import me.alanx.ecomer.web.dto.order.ReadableShopOrder;
import me.alanx.ecomer.web.dto.order.ShopOrder;
import me.alanx.ecomer.web.dto.shoppingcart.ShoppingCartData;
import me.alanx.ecomer.web.populator.customer.ReadableCustomerDeliveryAddressPopulator;
import me.alanx.ecomer.web.populator.order.ReadableOrderTotalPopulator;
import me.alanx.ecomer.web.populator.order.ReadableShippingSummaryPopulator;
import me.alanx.ecomer.web.populator.order.ReadableShopOrderPopulator;
	...
	public String displayCheckout(@CookieValue("cart") String cookie, Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
	    ...
	    if(customer!=null) {
			if(cart.getCustomerId()!=customer.getId().longValue()) {
				...
				AnonymousCustomer anonymousCustomer = (AnonymousCustomer)request.getAttribute(ApplicationConstants.ANONYMOUS_CUSTOMER);
...
/*					if(quote.getQuoteInformations() != null && quote.getQuoteInformations().size() >0) {
	...
	public @ResponseBody ReadableShopOrder calculateOrderTotal(@ModelAttribute(value="order") ShopOrder order, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
		...
		try {
			...
			//re-generate cart
			 me.alanx.ecomer.core.model.shoppingcart.ShoppingCart cart = shoppingCartFacade.getShoppingCartModel(shoppingCartCode, store);
