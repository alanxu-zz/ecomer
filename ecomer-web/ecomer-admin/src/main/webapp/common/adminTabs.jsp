  						...
  						<c:forEach items="${currentMenu.menus}" var="menu">
  							...
  							<c:choose>
  							    <c:when test="${fn:length(menu.menus)==0}">
  									<li id="${menu.code}-tab" <c:if test="${activeMenus[menu.code]!=null}"> class="active"</c:if>><a href="${menu.url}" id="${menu.code}-link" data-toggle="tab"><s:message code="menu.${menu.code}" text="${menu.code}"/></a></li>
  							    </c:when>
  							    <c:otherwise>
  										...
  										<ul class="dropdown-menu"> 
  											<c:forEach items="${menu.menus}" var="submenu">
  												<sec:authorize access="hasRole('${submenu.role}') and fullyAuthenticated">
  													<li><a href="${submenu.url}" id="${submenu.code}-link" data-toggle="tab"><s:message code="menu.${submenu.code}" text="${submenu.code}"/></a></li>
  												</sec:authorize>
  											</c:forEach>
  										</ul> 
  									</li>
