...
import me.alanx.ecomer.core.model.catalog.product.ProductImageDescription;
...
public class ProductController {
	...
	private String displayProduct(Long productId, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		me.alanx.ecomer.web.dto.admin.catalog.Product product = new me.alanx.ecomer.web.dto.admin.catalog.Product();
					...
					if(availability.getRegion().equals(me.alanx.ecomer.core.constants.Constants.ALL_REGIONS)) {
						...
						for(ProductPrice price : prices) {
							if(price.isDefaultPrice()) {
								productPrice = price;
								product.setProductPrice(ProductPriceUtils.getAdminFormatedAmount(store, productPrice.getProductPriceAmount()));
							}
						}
					}
				}
	...
	public String saveProduct(@Valid @ModelAttribute("product") me.alanx.ecomer.web.dto.admin.catalog.Product  product, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
		...
		try {
			submitedPrice = ProductPriceUtils.getAmount(product.getProductPrice());
		} catch (Exception e) {
	...
	public String duplicateProduct(@ModelAttribute("productId") Long  id, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
		...
		for(ProductAvailability pAvailability : dbProduct.getAvailabilities()) {
			...
			for(ProductPrice pPrice : prices) {
				...
				if(price.isDefaultPrice()) {
					product.setPrice(price);
					product.setProductPrice(ProductPriceUtils.getAdminFormatedAmount(store, price.getProductPriceAmount()));
				}
		...
		//attributes
		Set<ProductOptionPrice> attributes = new HashSet<ProductOptionPrice>();
		for(ProductOptionPrice pAttribute : dbProduct.getOptions()) {
			
			ProductOptionPrice attribute = new ProductOptionPrice();
			attribute.setIsDefault(pAttribute.isDefault());
			attribute.setDiscounted(pAttribute.isDiscounted());
			attribute.setDisplayOnly(pAttribute.isDisplayOnly());
			attribute.setRequired(pAttribute.isRequired());
			attribute.setOptionPrice(pAttribute.getOptionPrice());
			attribute.setIsFree(pAttribute.isFree());
			attribute.setWeight(pAttribute.getWeight());
		...
		newProduct.setOptions(attributes);
