...
import me.alanx.ecomer.core.model.shipping.CustomShippingQuoteWeightItem;
import me.alanx.ecomer.core.model.shipping.CustomShippingQuotesConfiguration;
import me.alanx.ecomer.core.model.shipping.CustomShippingQuotesRegion;
					...
					if(quote.getCustomRegionName().equals(region)) {
						...
						if(quoteItems!=null) {
							for(CustomShippingQuoteWeightItem quoteItem : quoteItems) {
								Map<String,String> entry = new HashMap<String,String> ();
								entry.put("price", ProductPriceUtils.getAdminFormatedAmountWithCurrency(store,quoteItem.getPrice()));
...
/*		List<Country> shippingCountries = new ArrayList<Country>();
		...
		List<String> environments = new ArrayList<String>();
		environments.add(me.alanx.ecomer.core.constants.Constants.PRODUCTION_ENVIRONMENT);//only production
