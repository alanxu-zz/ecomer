...
public class ProductOptionPriceController {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(ProductOptionPriceController.class);
	...
	public String displayProductOptionPrices(@RequestParam("id") long productId, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
	...
	public @ResponseBody ResponseEntity<String> pageAttributes(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			//List<ProductqAttribute> attributes = productqAttributeService.getByProductId(store, product, language);
			
			for(ProductOptionPrice attr : product.getOptions()) {
				...
				entry.put("display", attr.isDisplayOnly());
				entry.put("value", optionValueDescription.getName());
				entry.put("order", attr.getProductOptionSortOrder());
				entry.put("price", ProductPriceUtils.getAdminFormatedAmountWithCurrency(store,attr.getOptionPrice()));
	...
	private String displayAttribute(Long productId, Long id, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ProductOptionPrice attribute = null;
		...
		if(id!=null && id.intValue()!=0) {//edit mode
			...
			attribute = productOptionPriceService.getById(id);
			attribute.setAttributePrice(ProductPriceUtils.getAdminFormatedAmount(store, attribute.getOptionPrice()));
			attribute.setAdditionalWeight(String.valueOf(attribute.getWeight().intValue()));
			attribute.setAttributeSortOrder(String.valueOf(attribute.getProductOptionSortOrder()));
			
		} else {
			
			attribute = new ProductOptionPrice();
	...
	public String saveAttribute(@Valid @ModelAttribute("attribute") ProductOptionPrice attribute, BindingResult result, Model model, HttpServletRequest request, Locale locale) throws Exception {
		...
		ProductOptionPrice dbEntity =	null;	

		if(attribute.getId() != null && attribute.getId() >0) { //edit entry
			
			//get from DB
			dbEntity = productOptionPriceService.getById(attribute.getId());
		...
		try {
			submitedPrice = ProductPriceUtils.getAmount(attribute.getAttributePrice());
			attribute.setOptionPrice(submitedPrice);
		} catch (Exception e) {
		...
		//validate weight
		try {
			Integer weight = Integer.parseInt(attribute.getAdditionalWeight());
			attribute.setWeight(new BigDecimal(weight));
		} catch(Exception e) {
		...
		productOptionPriceService.saveOrUpdate(attribute);
	...
	public @ResponseBody ResponseEntity<String> deleteProductPrice(HttpServletRequest request, HttpServletResponse response, Locale locale) {
		...
		try {
			...
			ProductOptionPrice attribute = productOptionPriceService.getById(attributeId);
