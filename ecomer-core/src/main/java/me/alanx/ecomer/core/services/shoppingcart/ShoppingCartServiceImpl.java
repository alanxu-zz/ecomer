...
import me.alanx.ecomer.core.repositories.shoppingcart.ShoppingCartAttributeRepository;
import me.alanx.ecomer.core.repositories.shoppingcart.ShoppingCartItemRepository;
import me.alanx.ecomer.core.repositories.shoppingcart.ShoppingCartRepository;
...
/*					if (!item.isObsolete()) {
	...
	private void getPopulatedItem(final ShoppingCartItem item) throws Exception {
		...
		Set<ProductOptionPrice> productAttributes = product.getOptions();
		List<ProductOptionPrice> attributesList = new ArrayList<ProductOptionPrice>();//attributes maintained
		...
		if ( (productAttributes != null && productAttributes.size() > 0) || (cartAttributes != null && cartAttributes.size() > 0)) {
			for (ShoppingCartAttributeItem attribute : cartAttributes) {
				...
				for (ProductOptionPrice productAttribute : productAttributes) {
			...
			for (ShoppingCartItem shoppingCartItem : sessionCart.getLineItems()) {
				...
				if (CollectionUtils.isNotEmpty(cartAttributes)) {
					for (ShoppingCartAttributeItem shoppingCartAttributeItem : cartAttributes) {
						ProductOptionPrice productAttribute = productOptionPriceService
